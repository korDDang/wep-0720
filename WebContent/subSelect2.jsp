...
int cnt=0;
try{
	String sql="select count(*) from sub0720";
...
int no=0;
try{
	String sql="select l.idx,l.name,s.id,s.subject_name,s.credit,s.week,s.start_hour,s.end_hour from sub0720 s,lecturer0720 l where s.lecturer=l.idx order by l.idx asc,s.id asc";
	pstmt=conn.prepareStatement(sql);
	rs=pstmt.executeQuery();
	while(rs.next()){
		...
		String start_hou=rs.getString(7);
